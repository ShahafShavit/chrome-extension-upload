...
function createChromeWebstoreV2Client(options: {
	...
	async function uploadExisting(
		...
		if (!res.ok) {
			...
			return failureItem(
				String(ge?.error?.code ?? res.status),
				msg,
				extensionId
			)
		}
	...
	async function get(_projection?: string): Promise<ItemResourceLike> {
		...
		if (!res.ok) {
			...
			return {
				...
				itemError: [
					{
						error_code: String(ge?.error?.code ?? res.status),
						error_detail: msg
					}
				],
				publishState: 'error',
...
function createChromeWebstoreV1Client(options: {
	...
	const publishURI = (id: string, target: string): string => {
		const url = new URL(
			`${V1_ROOT}/chromewebstore/v1.1/items/${id}/publish`
		)
